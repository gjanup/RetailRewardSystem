...
{
    public class RewardSystemDataAccess : IRewardSystemDataAccess
    {
        ...
        public List<Transaction> GetTransactions(Customer customer)
        {
            ...
            using (var _dbContext = new RetailRewardSystemDataContext())
            {
                transactions = _dbContext.Transactions.Where(t => t.CustomerId == customer.Id).ToList();
            }
        ...
        public bool IsCustomerPresent(Customer customer)
        {
            using (var _dbContext = new RetailRewardSystemDataContext())
            { 
                if(_dbContext.Customers.Any(c=>c.Name == customer.Name && c.PhoneNumber == customer.PhoneNumber))
                    return true;
                return false;
            }
        }
